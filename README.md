...
要在一台电脑上测试群聊功能，有以下几种方法：

- **使用不同浏览器**：每个浏览器有独立的身份（推荐）
- **使用无痕模式**：每个无痕窗口有独立的身份
- **手动清除身份**：在同一浏览器中清除 localStorage 后重新加载

详细测试指南请参考：[测试指南](./docs/testing-guide.md)

测试辅助工具：访问 `http://localhost:5173/test-helper.html` 可以快速查看和管理身份信息。
...
本项目实现了一个基于 Waku 协议的迷你加密聊天应用，展示了去中心化通信的核心概念和技术实现。虽然功能和安全性还有提升空间，但已经覆盖了基本的聊天功能和安全需求，可作为学习Waku协议和去中心化通信的参考。
